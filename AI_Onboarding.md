...
|------|---------|-------------|
| Feb 12, 2026 | **Capterra PPC Landing Pages + HubSpot CRM Sync** - 20 category-specific landing pages at `/capterra/[slug]` for Capterra listings (IoT, Compliance, Audit, EHS, Food Service, Inspection, Quality Management, etc.). Isolated PPC layout with noindex/nofollow, no navigation, no external links. Conversion-focused design with dark gradient hero, live sensor mosaic, glassmorphism benefit cards, platform capabilities, ROI section with dashboard mockup, testimonials, industries, differentiators, FAQ accordion, and bottom CTA form section. Native form with full UTM attribution tracking. `ppc_leads` DB table for lead storage. HubSpot CRM auto-sync (create/update contacts with attribution data). Confirmation email to leads + internal notification. Capterra measurement pixel + conversion event firing on submit. `robots.txt` blocking crawlers. `lib/hubspot.ts` for CRM integration. `lib/ppc-config.ts` with all 20 category definitions. Requires `HUBSPOT_ACCESS_TOKEN` env var in Vercel. | AI |
...
- **Profile badge** - Red notification badge on profile avatar when admin has pending reviews
