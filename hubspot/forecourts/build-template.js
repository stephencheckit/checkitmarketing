...
body = body.replaceAll(
  'https://images.cdn.autocar.co.uk/sites/autocar.co.uk/files/images/car-reviews/first-drives/legacy/10_bp_forecourt_high_resolution.jpg',
  '{{ hero_image }}'
);
// URL lives inside style="background-image:url(&quot;...&quot;)" — match up to
// (but not including) the closing &quot;) so the quote survives.
body = body.replace(
  /https:\/\/images\.unsplash\.com\/photo-1516319915504[^)]*?(?=&quot;\))/g,
  '{{ journey_image }}'
);
body = body.replaceAll('https://checkitv6.com/checkit%20v6-1.webp', '{{ platform_image }}');

// Secondary CTA: slate-gray -> corporate teal
body = body
  .replaceAll('bg-slate-100!', 'bg-teal-50!')
  .replaceAll('text-slate-700!', 'text-teal-700!')
  .replaceAll('hover:bg-slate-200!', 'hover:bg-teal-100!');
...
{{ require_css(get_asset_url("./forecourts.css")) }}
...
const leftover = body.match(/https:\/\/(images\.cdn\.autocar|images\.unsplash|checkitv6)[^"')]*/g);
